...
local configPath = "ReveliX.json"
local configData

if isfile(configPath) then
    configData = json:JSONDecode(readfile(configPath))
else
    configData = { keysystem = "true" }
    writefile(configPath, json:JSONEncode(configData))
end

local isKeySystemActive = configData.keysystem == "true"

local function saveConfig()
    writefile(configPath, json:JSONEncode(configData))
end

if not isKeySystemActive then
    ...
    if isfile("Key.txt") and (Config:Verify_Key(readfile("Key.txt")) == trueData or Config:Verify_PremiumKey(readfile("Key.txt")) == trueData) then
        ...
        CheckKey.BackgroundColor3 = Color3.fromRGB(62, 67, 76)
        CheckKey.BorderColor3 = Color3.fromRGB(0, 0, 0)
        CheckKey.BorderSizePixel = 0
        CheckKey.Position = UDim2.new(0.0694980696, 0, 0.59632355, 0)
        CheckKey.Size = UDim2.new(0, 102, 0, 50)
        CheckKey.AutoButtonColor = false
        CheckKey.Font = Enum.Font.SourceSans
        CheckKey.Text = "Check key"
        CheckKey.TextColor3 = Color3.fromRGB(254, 254, 254)
        ...
        resultLabel.Text = "just do the key lazy"
